...
function fn_epayph_delete_payment_processors()
{
    db_query("DELETE FROM ?:payment_descriptions WHERE payment_id IN (SELECT payment_id FROM ?:payments WHERE processor_id IN (SELECT processor_id FROM ?:payment_processors WHERE processor_script IN ('epayph.php')))");
    db_query("DELETE FROM ?:payments WHERE processor_id IN (SELECT processor_id FROM ?:payment_processors WHERE processor_script IN ('epayph.php'))");
    db_query("DELETE FROM ?:payment_processors WHERE processor_script IN ('epayph.php')");
}

function fn_epayph_get_checkout_payment_buttons(&$cart, &$cart_products, &$auth, &$checkout_buttons, &$checkout_payments, &$payment_id)
{
...
function fn_epayph_payment_url(&$method, &$script, &$url, &$payment_dir)
{
    //if (strpos($script, 'paypal_express.php') !== false) {
        $payment_dir = '/app/addons/epayph/payments/';
    //}
}
...
function fn_get_epayph_settings($lang_code = DESCR_SL)
{
    $pp_settings = Settings::instance()->getValues('epayph', 'ADDON');
    if (!empty($pp_settings['general']['pp_statuses'])) {
        $pp_settings['general']['pp_statuses'] = unserialize($pp_settings['general']['pp_statuses']);
    }

    $pp_settings['general']['main_pair'] = fn_get_image_pairs(fn_epayph_get_logo_id(), 'epayph_logo', 'M', false, true, $lang_code);

    return $pp_settings['general'];
}

function fn_epayph_get_logo_id()
{
...
function fn_epayph_update_payment_pre(&$payment_data, &$payment_id, &$lang_code, &$certificate_file, &$certificates_dir)
{
    if (!empty($payment_data['processor_id']) && db_get_field("SELECT processor_id FROM ?:payment_processors WHERE processor_id = ?i AND processor_script IN ('epayph.php')", $payment_data['processor_id'])) {
        ...
        if (!empty($p_surcharge) || !empty($a_surcharge)) {
            ...
            fn_set_notification('E', __('error'), __('text_epayph_surcharge'));
        }
    }
}

function fn_epayph_rma_update_details_post(&$data, &$show_confirmation_page, &$show_confirmation, &$is_refund, &$_data, &$confirmed)
{
        ...
        if ($change_return_status['status_to'] != $change_return_status['status_from'] && $st_inv[$change_return_status['status_to']]['params']['inventory'] != 'D') {
            if (!empty($order_info['payment_method']) && !empty($order_info['payment_method']['processor_params']) && !empty($order_info['payment_info']) && !empty($order_info['payment_info']['transaction_id'])) {
                if (!empty($order_info['payment_method']['processor_params']['username']) && !empty($order_info['payment_method']['processor_params']['password'])) {
                    ...
                    fn_epayph_build_request($order_info['payment_method'], $request_data, $post_url, $cert_file);
                    $result = fn_epayph_request($request_data, $post_url, $cert_file);
                }
            }
        }
    }
...
function fn_validate_epayph_order_info($data, $order_info)
{
    ...
    if (!isset($data['num_cart_items']) || count($order_info['products']) != $data['num_cart_items']) {
        if (isset($order_info['payment_method']) && isset($order_info['payment_method']['processor_id']) && 'epayph.php' == db_get_field("SELECT processor_script FROM ?:payment_processors WHERE processor_id = ?i", $order_info['payment_method']['processor_id'])) {
    ...
    if (!isset($order_info['payment_method']['processor_params']) || !isset($order_info['payment_method']['processor_params']['currency']) || !isset($data['mc_currency']) || $data['mc_currency'] != $order_info['payment_method']['processor_params']['currency']) {
        //if cureency defined in epayph settings do not match currency in IPN
...
function fn_epayph_get_customer_info($data)
{
    $user_data = array();
...
function fn_pp_get_ipn_order_ids($data)
{
    ...
    fn_set_hook('epayph_get_ipn_order_ids', $data, $order_ids);

    return $order_ids;
}

function fn_epayph_prepare_checkout_payment_methods(&$cart, &$auth, &$payment_groups)
{
...
function fn_pp_standart_prepare_products($order_info, $epayph_currency = '', $max_pp_products = MAX_EPAYPH_PRODUCTS)
{
    $post_data = array();
    $product_count = 1;

    if (empty($epayph_currency)) {
        $epayph_currency = !empty($order_info['payment_method']['processor_params']['currency']) ? $order_info['payment_method']['processor_params']['currency'] : CART_PRIMARY_CURRENCY;
    }
    ...
    if (empty($order_info['use_gift_certificates']) && !floatval($order_info['subtotal_discount']) && empty($order_info['points_info']['in_use']) && count($order_info['products']) < MAX_EPAYPH_PRODUCTS) {
        $i = 1;
        if (!empty($order_info['products'])) {
            foreach ($order_info['products'] as $k => $v) {
                $suffix = '_'.($i++);
                $v['product'] = htmlspecialchars(strip_tags($v['product']));
                $v['price'] = fn_format_price(($v['subtotal'] - fn_external_discounts($v)) / $v['amount'], $epayph_currency);
        ...
        if (!empty($order_info['taxes']) && Registry::get('settings.General.tax_calculation') == 'subtotal') {
            foreach ($order_info['taxes'] as $tax_id => $tax) {
                ...
                $item_price = fn_format_price($tax['tax_subtotal'], $epayph_currency);
        ...
        if (!empty($order_info['gift_certificates'])) {
            foreach ($order_info['gift_certificates'] as $k => $v) {
                ...
                $v['amount'] = (!empty($v['extra']['exclude_from_calculate'])) ? 0 : fn_format_price($v['amount'], $epayph_currency);
        ...
        if ($take_surcharge && floatval($order_info['payment_surcharge'])) {
            $suffix = '_' . ($i++);
            $name = __('surcharge');
            $payment_surcharge_amount = fn_format_price($order_info['payment_surcharge'], $epayph_currency);
            $post_data["item_name$suffix"] = $name;
            $post_data["amount$suffix"] = $payment_surcharge_amount;
            $post_data["quantity$suffix"] = '1';
        }
        $product_count = $i - 1;
    } elseif ($epayph_total <= 0) {
        $post_data['item_name_1'] = __('total_product_cost');;
        $post_data['amount_1'] = fn_format_price($order_info['total'], $epayph_currency);
        $post_data['quantity_1'] = '1';
        $post_data['amount'] = fn_format_price($order_info['total'], $epayph_currency);;
        $post_data['shipping_1'] = 0;
    } else {
        $post_data['item_name_1'] = __('total_product_cost');;
        $post_data['amount_1'] = $epayph_total;
        $post_data['quantity_1'] = '1';
    }
